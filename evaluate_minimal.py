...
# ==========================================
# 2. ЗАГРУЗКА НАШЕГО RAG (БЕЗ RERANKER)
# ==========================================
print("Загружаем базу данных и модель эмбеддингов...")
...
def ask_rag(question: str):
    """Прогоняет вопрос через упрощенный RAG (без реранкера)."""
    query_embedding = retriever_model.encode([question])[0].tolist()

    # Берем только 1 лучший чанк (без реранкинга)
...
def f1_score(prediction, reference):
    """F1-мера между ответом и эталоном."""
...
# ==========================================
print("\nПрогоняем вопросы через RAG (это займет 2-3 минуты)...\n")
metrics = {
    "faithfulness": [],
    "context_relevance": [],
    "completeness": [],
...
print(f"  Context Relevance (Релев. контекст): {avg_cr:.3f}")
print(f"  Completeness (Полнота ответа):       {avg_comp:.3f}")
print(f"  F1 vs Ground Truth (Точность):       {avg_f1:.3f}")
